        ...
        if (ab.get(`config.${user.guild.id}.antiban`) === true) {
            ...
            if (!perm) {
                ...
                if (punish.get(`sanction_${user.guild.id}`) === "ban") {
                    ...
                    user.guild.members.resolve(action.executor).roles.cache.forEach(role => {
                        if (role.name !== '@everyone') {
                            user.guild.members.resolve(action.executor).roles.remove(role).catch(err => { throw err })
                        }
                    })
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                client.channels.cache.get(rlog.fetch(`${user.guild.id}.raidlog`)).send({ embeds: [embed] }).catch(console.error)
