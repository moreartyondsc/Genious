...
module.exports = {
    ...
    async execute(client, message, channel) {
        ...
        if (owner.get(`owners.${message.member.id}`) || wl.get(`${message.guild.id}.wl`) || config.app.owners === message.author.id === true) return
        ...
        if (antilinkinvite == true) {
            ...
            if (isLink == true) {
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                client.channels.cache.get(rlog.fetch(`${message.guild.id}.raidlog`)).send({ embeds: [embed] }).catch(console.error)
            }
