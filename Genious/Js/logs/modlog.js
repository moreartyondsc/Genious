...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (modlog.get(`${message.guild.id}.modlog`) === newChannel) return message.channel.send(`<:CircleSearch:984992107158728734>・__Nouveau salon des logs de modération :__ \`${modlog.get(`${message.guild.id}.modlog`)}\``)
            else {
                modlog.set(`${message.guild.id}.modlog`, newChannel.id)
                message.channel.send(`<:CircleSearch:984992107158728734>・__Nouveau salon des logs de modération :__ ${args[0]}`)
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`<:CircleSearch:984992107158728734> Ce salon est désormais utilisé pour __toutes__ les **logs de modération** du serveur\n Executeur : <@${message.author.id}>`)
                    .setTimestamp()
