...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (raidlog.get(`${message.guild.id}.raidlog`) === newChannel) return message.channel.send(`<:ModoDiscord:959184555531698186>・__Nouveau salon des logs de raid :__ \`${raidlog.get(`${message.guild.id}.raidlog`)}\``)
            else {
