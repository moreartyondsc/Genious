...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            if (msglog.get(`${message.guild.id}.messagelog`) === newChannel) return message.channel.send(`<:CircleSearch:984992107158728734>・__Nouveau salon des logs message :__ \`${msglog.get(`${message.guild.id}.messagelog`)}\``)
            else {
                msglog.set(`${message.guild.id}.messagelog`, newChannel.id)
                message.channel.send(`<:CircleSearch:984992107158728734>・__Nouveau salon des logs :__ ${args[0]}`)
                ...
                const embed = new Discord.MessageEmbed()
                    ...
                    .setDescription(`<:CircleSearch:984992107158728734> Ce salon est désormais utilisé pour __toutes__ les **logs message** du serveur\n Executeur : <@${message.author.id}>`)
                    .setTimestamp()
