    ...
    async execute(client, message, args) {
        ...
        if (owner.get(`owners.${message.author.id}`) || config.app.owners.includes(message.author.id) || config.app.funny.includes(message.author.id) === true) {
            ...
            const alert = new Discord.MessageEmbed()
                ...
                .setDescription(`<a:Abus:957097615147810956> Toutes les personnes possédant des permissions **Dangereuses** ont étaient derank\n Executeur : <@${message.author.id}>`)
                .setTimestamp()
                .setFooter({ text: `⚠️ ${footer}` })
            client.channels.cache.get(channellogs).send({ content: `${roleping}`, embeds: [alert] }).catch(console.error)
        }
    }
}